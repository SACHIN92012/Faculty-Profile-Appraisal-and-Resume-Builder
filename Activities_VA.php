...
if (Input::exists() && isset($_POST['csubmit'])) {
  ...
  try {
    ...
    $stmt = "INSERT INTO `faculty_profile_activities_va` (`sno`, `name`, `roll`, `dept`, `placeOfVisit`, `purpose`, `duration`, `activityDate`, `aastUpdated`) VALUES (NULL, '$fname', '$roll', '$dept', '$pov', '$purpose', $duration, '$date',NULL)";
...
if (Input::exists() && isset($_POST['delete_entry'])) {
  ...
  try {
    ...
    // $stmt="insert into discussion values('$email','$dateTime','$club_id','$text');";
    $stmt = "Delete from `faculty_profile_activities_va` where placeOfVisit='$pov' and purpose='$purpose' and roll='$roll' and duration=$duration and activityDate = '$date'";
    $result = mysqli_query($conn, $stmt);
  } catch (Exception $e) {
    die($e->getMessage());
  }
...
if (Input::exists() && isset($_POST['upgrade_entry_x'])) {
  ...
  try {
    ...
    $stmt = "select * from faculty_profile_Activities_va where sno>$SNO and roll=$roll order by sno asc limit 1";
    // echo $stmt;
    $result = mysqli_query($conn, $stmt);
    $count = 0;
    $val = 0;
    ...
    // echo $val;
    if ($count != 0) {
      $stmt = "update faculty_profile_Activities_va set sno=-1 where sno=$val";
      $result = mysqli_query($conn, $stmt);
      $stmt = "update faculty_profile_activities_va set sno=$val where sno=$SNO";
      $result = mysqli_query($conn, $stmt);
      $stmt = "update faculty_profile_activities_va set sno=$SNO where sno=-1";
      $result = mysqli_query($conn, $stmt);
    }
    // $result=mysqli_query($conn,$stmt);
  } catch (Exception $e) {
...
if (Input::exists() && isset($_POST['edit'])) {
  ...
  try {
    ...
    $prev_pov = Input::get('prev_pov');
    $prev_purpose = Input::get('prev_purpose');
    $prev_duration = Input::get('prev_duration');
    $prev_date = Input::get('prev_activityDate');
    ...
    $stmt = "Update `faculty_profile_activities_va` set placeOfVisit='$pov', purpose='$purpose' , duration=$duration , activityDate = '$date' where placeOfVisit='$prev_pov' and purpose='$prev_purpose' and roll='$roll' and duration=$prev_duration and activityDate = '$prev_date'";
    // echo "<br><br><br><br><br><br><br><br>";
    // echo $pov;
    // echo $purpose;
    // echo $duration;
...
<body>
          ...
          <div class="row">
            ...
            <div class="col-md-12" style="width: 65rem;">
              ...
              ?>
                ...
                <div class="card">
                  <div class="card-header">
                    <h4><i class='fa fa-edit'></i>Edit Visit Abroad</h4>
                  </div>
                  <br>
                  <form action="Activities_VA.php" method="post">
                    <input type='hidden' name='prev_pov' value=<?php echo "'$pov'" ?>>
                    <input type='hidden' name='prev_purpose' value=<?php echo "'$purpose'" ?>>
                    <input type='hidden' name='prev_duration' value=<?php echo "'$duration'" ?>>
                    <input type='hidden' name='prev_activityDate' value=<?php echo "'$date'" ?>>
                    <div class="form-group">
                      <label>Place of Visit<span class="m-1 text-primary">*</span></label>
                      <input type="text" value=<?php echo "'$pov'" ?> class="form-control" name="pov" id="pov" placeholder="Place of visit" required>
                    </div>
                    <div class="form-group">
                      <label>Purpose<span class="m-1 text-primary">*</span></label>
                      <input type="text" value=<?php echo "'$purpose'" ?> class="form-control" name="purpose" id="purpose" placeholder="Purpose" required>
                    </div>
                    <div class="form-group">
              ...
              <?php
                ...
                <div class="card">
                  <div class="card-header">
                    <h4><i class="fa fa-plus"></i> Visit Abroad</h4>
                  </div>
                  <br>
                  <form action="Activities_VA.php" method="post">
                    ...
                    <div class="form-group">
                      <label>Place of Visit<span class="m-1 text-primary">*</span></label>
                      <input type="text" class="form-control" name="pov" id="pov" placeholder="Place of visit" required>
                    </div>
                    <div class="form-group">
                      <label>Purpose<span class="m-1 text-primary">*</span></label>
                ...
                <table class="table table-striped table-hover">
                  ...
                  <tbody>
                    ...
                    if (!$conn)
                      die("Unable to connect to database");
                    // echo $roll;
                    $stmt = "select * from faculty_profile_activities_va where roll='$roll' order by sno desc;";
                    // echo $stmt;
                    $result = mysqli_query($conn, $stmt);
                    while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                      <tr>
                        <td>
                          ...
                          <form action="Activities_VA.php" method="post">
                            <input type='hidden' name='pov' value=<?php echo "'";
                                                                  echo $row['placeOfVisit'];
                                                                  echo "'"; ?>>
                            <input type='hidden' name='purpose' value=<?php echo "'";
                                                                      echo $row['purpose'];
                                                                      echo "'"; ?>>
  ...
  <script src="js/activities/Activities_VA.js"></script>
</body>
