...
if (Input::exists() && isset($_POST['csubmit'])) {
  ...
  try {
    ...
    $role = Input::get('role');
    $noOfParticipants = Input::get('noOfParticipants');
    $courseBudget = Input::get('courseBudget');
    $type = Input::get('type');
    ...
    $stmt = "INSERT INTO `faculty_profile_activities_stc` (`sno`, `name`, `roll`, `dept`,`title`, `duration`, `noOfParticipants`,`courseBudget`,`type`,`role`, `activityDate`, `lastUpdated`) VALUES (NULL, '$fname', '$roll', '$dept', '$title', $duration,$noOfParticipants,$courseBudget, '$type', '$role', '$activityDate',NULL)";
...
if (Input::exists() && isset($_POST['delete_entry'])) {
  ...
  try {
    ...
    // $stmt="insert into discussion values('$email','$dateTime','$club_id','$text');";
    $stmt = "Delete from `faculty_profile_activities_stc` where title='$title' and duration='$duration' and role='$role' and roll='$roll' and noOfParticipants=$noOfParticipants and activityDate = '$activityDate' and type='$type' and courseBudget=$courseBudget ";
    $result = mysqli_query($conn, $stmt);
  } catch (Exception $e) {
    die($e->getMessage());
  }
...
if (Input::exists() && isset($_POST['upgrade_entry_x'])) {
  ...
  try {
    ...
    $stmt = "select * from faculty_profile_Activities_stc where sno>$SNO and roll=$roll order by sno asc limit 1";
    // echo $stmt;
    $result = mysqli_query($conn, $stmt);
    $count = 0;
    $val = 0;
    ...
    // echo $val;
    if ($count != 0) {
      $stmt = "update faculty_profile_Activities_stc set sno=-1 where sno=$val";
      $result = mysqli_query($conn, $stmt);
      $stmt = "update faculty_profile_activities_stc set sno=$val where sno=$SNO";
      $result = mysqli_query($conn, $stmt);
      $stmt = "update faculty_profile_activities_stc set sno=$SNO where sno=-1";
      $result = mysqli_query($conn, $stmt);
    }
    // $result=mysqli_query($conn,$stmt);
  } catch (Exception $e) {
...
if (Input::exists() && isset($_POST['edit'])) {
  ...
  try {
    ...
    $prev_title = Input::get('prev_title');
    $prev_duration = Input::get('prev_duration');
    $prev_role = Input::get('prev_role');
    $prev_noOfParticipants = Input::get('prev_noOfParticipants');
    $prev_courseBudget = Input::get('prev_courseBudget');
    $prev_type = Input::get('prev_type');
    ...
    // $stmt="insert into discussion values('$email','$dateTime','$club_id','$text');";
    $stmt = "Update `faculty_profile_activities_stc` set title='$title', duration=$duration, role = '$role', noOfParticipants=$noOfParticipants, courseBudget=$courseBudget, type='$type', activityDate='$activityDate' where title='$prev_title' and duration=$prev_duration and role='$prev_role' and noOfParticipants=$prev_noOfParticipants and courseBudget=$prev_courseBudget and type='$prev_type' and roll='$roll' and activityDate = '$prev_activityDate'";
...
<body>
 ...
 <nav class="navbar navbar-expand-sm navbar-dark bg-dark p-2 fixed-top">
            ...
            <div class="col-md-12" style="width: 65rem;">
              ...
              ?>
                ...
                <div class="card">
                  ...
                  <br>
                  <form action="Activities_STC.php" method="post">
                    <input type='hidden' name='prev_title' value=<?php echo "'$title'" ?>>
                    <input type='hidden' name='prev_duration' value=<?php echo "'$duration'" ?>>
                    <input type='hidden' name='prev_role' value=<?php echo "'$role'" ?>>
                    <input type='hidden' name='prev_noOfParticipants' value=<?php echo "'$noOfParticipants'"; ?>>
                    <input type='hidden' name='prev_courseBudget' value=<?php echo "'$courseBudget'" ?>>
                    <input type='hidden' name='prev_type' value=<?php echo "'$type'" ?>>
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" value=<?php echo "'$title'" ?> class="form-control" name="title" id="title" placeholder="Title" required>
                    </div>
                    <div class="form-group">
                      <label>Duration(In Days)<span class="m-1 text-primary">*</span></label>
                      <input type="number" value=<?php echo "'$duration'" ?> class="form-control" name="duration" id="duration" placeholder="Duration" required>
                    </div>
                    <div class="form-group">
                      <label>Number of Participants<span class="m-1 text-primary">*</span></label>
                      <input type="number" class="form-control" value=<?php echo "'$noOfParticipants'" ?> name="noOfParticipants" id="noOfParticipants" placeholder="No Of Participants" required>
                    </div>
                    <div class="form-group">
                      <label>Course Budget (In INR) <span class="m-1 text-primary">*</span></label>
                      <input type="number" class="form-control" value=<?php echo "'$courseBudget'" ?> name="courseBudget" id="courseBudget" placeholder="Course Budget" required>
                    </div>
                    <div class="form-group">
                      <label>Type<span class="m-1 text-primary">*</span></label>
                      <input type="text" class="form-control" value=<?php echo "'$type'" ?> name="type" id="type" placeholder="Type" required>
                    </div>
                    <div class="form-group">
                      <label>Role<span class="m-1 text-primary">*</span></label>
                      <input type="text" class="form-control" value=<?php echo "'$role'" ?> name="role" id="role" placeholder="Role" required>
                    </div>
              ...
              <?php
                ...
                <div class="card">
                  <div class="card-header">
                    <h4><i class="fa fa-plus"></i> Short Term Courses</h4>
                  </div>
                  <br>
                  <form action="Activities_STC.php" method="post">
                    ...
                    <div class="form-group">
                      <label>Number of Participants<span class="m-1 text-primary">*</span></label>
                      <input type="number" class="form-control" name="noOfParticipants" id="noOfParticipants" placeholder="No Of Participants" required>
                    </div>
                    <div class="form-group">
                      <label>Course Budget (In INR) <span class="m-1 text-primary">*</span></label>
                      <input type="number" class="form-control" name="courseBudget" id="courseBudget" placeholder="Course Budget" required>
                    </div>
                    <div class="form-group">
                      <label>Type<span class="m-1 text-primary">*</span></label>
                ...
                <table class="table table-striped table-hover">
                  ...
                  <tbody>
                    ...
                    if (!$conn)
                      die("Unable to connect to database");
                    // echo $roll;
                    $stmt = "select * from faculty_profile_activities_stc where roll='$roll' order by sno desc;";
                    // echo $stmt;
                    $result = mysqli_query($conn, $stmt);
                    while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                      <tr>
                        <td>
                          ...
                          <form action="Activities_STC.php" method="post">
                            ...
                            <input type='hidden' name='duration' value=<?php echo "'";
                                                                        echo $row['duration'];
                                                                        echo "'"; ?>>
                            <input type='hidden' name='noOfParticipants' value=<?php echo "'";
                                                                                echo $row['noOfParticipants'];
                                                                                echo "'"; ?>>
                            <input type='hidden' name='courseBudget' value=<?php echo "'";
                                                                            echo $row['courseBudget'];
                                                                            echo "'"; ?>>
                            <input type='hidden' name='type' value=<?php echo "'";
                                                                    echo $row['type'];
                                                                    echo "'"; ?>>
                            <input type='hidden' name='role' value=<?php echo "'";
                                                                    echo $row['role'];
                                                                    echo "'"; ?>>
                        ...
                        <td><?php echo $row['courseBudget'] ?></td>
  ...
  <script src="js/activities/Activities_STC.js"></script>
</body>
