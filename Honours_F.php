...
if (Input::exists() && isset($_POST['csubmit'])) {
  ...
  try {
    ...
    $fellowshipTitle = Input::get('fellowshipTitle');
    $year = Input::get('year');
    ...
    $stmt = "INSERT INTO `faculty_profile_honours_f` (`sno`, `name`, `roll`, `dept`, `fellowshipTitle`,`year`, `activityDate`, `lastUpdated`) VALUES (NULL, '$fname', '$roll', '$dept', '$fellowshipTitle', $year, '$activityDate',NULL)";
    // echo $stmt;
...
if (Input::exists() && isset($_POST['delete_entry'])) {
  ...
  try {
    ...
    // $stmt="insert into discussion values('$email','$dateTime','$club_id','$text');";
    $stmt = "Delete from `faculty_profile_honours_f` where fellowshipTitle='$fellowshipTitle' and year=$year and roll='$roll' and activityDate = '$activityDate'";
    $result = mysqli_query($conn, $stmt);
  } catch (Exception $e) {
    die($e->getMessage());
  }
...
if (Input::exists() && isset($_POST['upgrade_entry_x'])) {
  ...
  try {
    ...
    $stmt = "select * from faculty_profile_honours_f where sno>$SNO and roll=$roll order by sno asc limit 1";
    // echo $stmt;
    $result = mysqli_query($conn, $stmt);
    $count = 0;
    $val = 0;
    ...
    // echo $val;
    if ($count != 0) {
      $stmt = "update faculty_profile_honours_f set sno=-1 where sno=$val";
      $result = mysqli_query($conn, $stmt);
      $stmt = "update faculty_profile_honours_f set sno=$val where sno=$SNO";
      $result = mysqli_query($conn, $stmt);
      $stmt = "update faculty_profile_honours_f set sno=$SNO where sno=-1";
      $result = mysqli_query($conn, $stmt);
    }
    // $result=mysqli_query($conn,$stmt);
  } catch (Exception $e) {
...
if (Input::exists() && isset($_POST['edit'])) {
  ...
  try {
    ...
    // $stmt="insert into discussion values('$email','$dateTime','$club_id','$text');";
    $stmt = "Update `faculty_profile_honours_f` set fellowshipTitle='$fellowshipTitle', year=$year, activityDate='$activityDate' where fellowshipTitle='$prev_nameOfTheBody' and year=$prev_yearAwarded and roll='$roll' and activityDate = '$prev_activityDate'";
...
<!-- MAIN BODY - SIDEBAR ends -->
            ...
            <div class="col-md-12" style="width: 65rem;">
              ...
              ?>
                ...
                <div class="card">
                  ...
                  <br>
                  <form action="Honours_F.php" method="post">
                    <input type='hidden' name='prev_nameOfTheBody' value=<?php echo "'$fellowshipTitle'" ?>>
                    ...
                    <div class="form-group">
                      <label>Fellowship Title<span class="m-1 text-primary">*</span></label>
                      <input type="text" value=<?php echo "'$fellowshipTitle'" ?> class="form-control" name="fellowshipTitle" id="fellowshipTitle" placeholder="Fellowship Title" required>
                    </div>
                    <div class="form-group">
              ...
              <?php
                ...
                <div class="card">
                  <div class="card-header">
                    <h4><i class="fa fa-plus"></i> Fellowship (Only for 2020-21)</h4>
                  </div>
                  <br>
                  <form action="Honours_F.php" method="post">
                    ...
                    <div class="form-group">
                      <label>Fellowship Title<span class="m-1 text-primary">*</span></label>
                      <input type="text" class="form-control" name="fellowshipTitle" id="fellowshipTitle" placeholder="Fellowship Title" required>
                    </div>
                    <div class="form-group">
                      <label>Year<span class="m-1 text-primary">*</span></label>
                ...
                <table class="table table-striped table-hover">
                  ...
                  <tbody>
                    ...
                    if (!$conn)
                      die("Unable to connect to database");
                    // echo $roll;
                    $stmt = "select * from faculty_profile_honours_f where roll='$roll' order by sno desc;";
                    // echo $stmt;
                    $result = mysqli_query($conn, $stmt);
                    while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                      <tr>
                        <td>
                          ...
                          <form action="Honours_F.php" method="post">
                            <input type='hidden' name='fellowshipTitle' value=<?php echo "'";
                                                                              echo $row['fellowshipTitle'];
                                                                              echo "'"; ?>>
                        ...
                        <td><?php echo $row['fellowshipTitle'] ?></td>
  ...
  <script src="js/honours/Honours_F.js"></script>
</body>
