...
if (Input::exists() && isset($_POST['csubmit'])) {
  try {
    ...
    $stmt = "INSERT INTO `faculty_profile_activities_scw` (`sno`, `name`, `roll`, `dept`,`title`, `place`, `responsibility`, `noOfParticipants`, `activityDate`, `lastUpdated`) VALUES (NULL, '$fname', '$roll', '$dept', '$title', '$place','$responsibility', $noOfParticipants, '$activityDate',NULL)";
...
if (Input::exists() && isset($_POST['delete_entry'])) {
  try {
    ...
    // $stmt="insert into discussion values('$email','$dateTime','$club_id','$text');";
    $stmt = "Delete from `faculty_profile_activities_scw` where title='$title' and place='$place' and responsibility='$responsibility' and roll='$roll' and noOfParticipants=$noOfParticipants and activityDate = '$activityDate'";
    $result = mysqli_query($conn, $stmt);
    // // // echo if conference added successfully 
  } catch (Exception $e) {
    die($e->getMessage());
...
if (Input::exists() && isset($_POST['upgrade_entry_x'])) {
  try {
    ...
    $stmt = "select * from faculty_profile_Activities_scw where sno>$SNO and roll=$roll order by sno asc limit 1";
    // echo $stmt;
    $result = mysqli_query($conn, $stmt);
    $count = 0;
    $val = 0;
    ...
    // echo $val;
    if ($count != 0) {
      $stmt = "update faculty_profile_Activities_scw set sno=-1 where sno=$val";
      $result = mysqli_query($conn, $stmt);
      $stmt = "update faculty_profile_activities_scw set sno=$val where sno=$SNO";
      $result = mysqli_query($conn, $stmt);
      $stmt = "update faculty_profile_activities_scw set sno=$SNO where sno=-1";
      $result = mysqli_query($conn, $stmt);
    }
    // $result=mysqli_query($conn,$stmt);
  } catch (Exception $e) {
...
if (Input::exists() && isset($_POST['edit'])) {
  try {
    ...
    $prev_place = Input::get('prev_place');
    $prev_responsibility = Input::get('prev_responsibility');
    ...
    // $stmt="insert into discussion values('$email','$dateTime','$club_id','$text');";
    $stmt = "Update `faculty_profile_activities_scw` set title='$title', place='$place', responsibility='$responsibility' , noOfParticipants=$noOfParticipants , activityDate = '$activityDate' where title='$prev_title' and place='$prev_place' and responsibility='$prev_responsibility' and roll='$roll' and noOfParticipants=$prev_noOfParticipants and activityDate = '$prev_activityDate'";
    // echo $stmt;
    // echo "<br><br><br><br><br><br><br><br>";
    // echo $pov;
    // echo $purpose;
...
<head>
  <title>Seminar, Conferences, Workshops</title>
  <meta charset="utf-8">
...
<body>
            ...
            <div class="col-md-12" style="width: 65rem;">
              ...
              ?>
                ...
                <div class="card">
                  ...
                  <br>
                  <form action="Activities_SCW.php" method="post">
                    <input type='hidden' name='prev_title' value=<?php echo "'$title'" ?>>
                    <input type='hidden' name='prev_place' value=<?php echo "'$place'" ?>>
                    <input type='hidden' name='prev_responsibility' value=<?php echo "'$responsibility'"; ?>>
                    ...
                    <div class="form-group">
                      <label>Place<span class="m-1 text-primary">*</span></label>
                      <input type="text" value=<?php echo "'$place'" ?> class="form-control" name="place" id="place" placeholder="Place" required>
                    </div>
                    <div class="form-group">
                      <label>Responsibility<span class="m-1 text-primary">*</span></label>
                      <input type="text" value=<?php echo "'$responsibility'"; ?> class="form-control" name="responsibility" id="responsibility" placeholder="Responsibility" required>
                    </div>
                    <div class="form-group">
                      <label>Number of Participants<span class="m-1 text-primary">*</span></label>
                      <input type="number" value=<?php echo "$noOfParticipants" ?> class="form-control" name="noOfParticipants" id="noOfParticipants" placeholder="No Of Participants" required>
                    </div>
                    <div class="form-group">
                      <label for="student_activity_date">Activity Date:</label>
                      <input type="date" value=<?php echo "'$activityDate'" ?> id="student_activity_date" name="activityDate">
                    </div>
              ...
              <?php
                ...
                <div class="card">
                  <div class="card-header">
                    <h4><i class="fa fa-plus"></i> Seminar, Conference, Workshop</h4>
                  </div>
                  <br>
                  <form action="Activities_SCW.php" method="post">
                    ...
                    <div class="form-group">
                      <label>Responsibility<span class="m-1 text-primary">*</span></label>
                      <input type="text" class="form-control" name="responsibility" id="responsibility" placeholder="Responsibility" required>
                    </div>
                    <div class="form-group">
                      <label>Number of Participants<span class="m-1 text-primary">*</span></label>
                ...
                <table class="table table-striped table-hover">
                  ...
                  <tbody>
                    ...
                    if (!$conn)
                      die("Unable to connect to database");
                    // echo $roll;
                    $stmt = "select * from faculty_profile_activities_scw where roll='$roll' order by sno desc;";
                    // echo $stmt;
                    $result = mysqli_query($conn, $stmt);
                    while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                        ...
                        <td>
                          ...
                          <form action="Activities_SCW.php" method="post">
                            <input type='hidden' name='title' value=<?php echo "'";
                                                                    echo $row['title'];
                                                                    echo "'"; ?>>
                            <input type='hidden' name='place' value=<?php echo "'";
                                                                    echo $row['place'];
                                                                    echo "'"; ?>>
                            <input type='hidden' name='responsibility' value=<?php echo "'";
  ...
  <script src="js/activities/Activities_SCW.js"></script>
</body>
