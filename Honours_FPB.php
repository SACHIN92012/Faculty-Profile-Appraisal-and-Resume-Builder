...
if (Input::exists() && isset($_POST['csubmit'])) {
  ...
  try {
    ...
    $stmt = "INSERT INTO `faculty_profile_honours_fpb` (`sno`, `name`, `roll`, `dept`, `nameOfTheBody`,`yearAwarded`, `activityDate`, `lastUpdated`) VALUES (NULL, '$fname', '$roll', '$dept', '$nameOfTheBody', $yearAwarded, '$activityDate',NULL)";
...
if (Input::exists() && isset($_POST['delete_entry'])) {
  ...
  try {
    ...
    // $stmt="insert into discussion values('$email','$dateTime','$club_id','$text');";
    $stmt = "Delete from `faculty_profile_honours_fpb` where nameOfTheBody='$nameOfTheBody' and yearAwarded=$yearAwarded and roll='$roll' and activityDate = '$activityDate'";
    $result = mysqli_query($conn, $stmt);
  } catch (Exception $e) {
    die($e->getMessage());
  }
...
if (Input::exists() && isset($_POST['upgrade_entry_x'])) {
  ...
  try {
    ...
    $stmt = "select * from faculty_profile_honours_fpb where sno>$SNO and roll=$roll order by sno asc limit 1";
    // echo $stmt;
    $result = mysqli_query($conn, $stmt);
    $count = 0;
    $val = 0;
    ...
    // echo $val;
    if ($count != 0) {
      $stmt = "update faculty_profile_honours_fpb set sno=-1 where sno=$val";
      $result = mysqli_query($conn, $stmt);
      $stmt = "update faculty_profile_honours_fpb set sno=$val where sno=$SNO";
      $result = mysqli_query($conn, $stmt);
      $stmt = "update faculty_profile_honours_fpb set sno=$SNO where sno=-1";
      $result = mysqli_query($conn, $stmt);
    }
    // $result=mysqli_query($conn,$stmt);
  } catch (Exception $e) {
...
if (Input::exists() && isset($_POST['edit'])) {
  ...
  try {
    ...
    // $stmt="insert into discussion values('$email','$dateTime','$club_id','$text');";
    $stmt = "Update `faculty_profile_honours_fpb` set nameOfTheBody='$nameOfTheBody', yearAwarded=$yearAwarded, activityDate='$activityDate' where nameOfTheBody='$prev_nameOfTheBody' and yearAwarded=$prev_yearAwarded and roll='$roll' and activityDate = '$prev_activityDate'";
...
<head>
  <title>Fellow Professional Body</title>
  <meta charset="utf-8">
...
<body>
            ...
            <div class="col-md-12" style="width: 65rem;">
              ...
              ?>
                ...
                <div class="card">
                  ...
                  <br>
                  <form action="Honours_FPB.php" method="post">
              ...
              <?php
                ...
                <div class="card">
                  <div class="card-header">
                    <h4><i class="fa fa-plus"></i> Fellow - Professional Body</h4>
                  </div>
                  <br>
                  <form action="Honours_FPB.php" method="post">
                ...
                <table class="table table-striped table-hover">
                  ...
                  <tbody>
                    ...
                    if (!$conn)
                      die("Unable to connect to database");
                    // echo $roll;
                    $stmt = "select * from faculty_profile_honours_fpb where roll='$roll' order by sno desc;";
                    // echo $stmt;
                    $result = mysqli_query($conn, $stmt);
                    while ($row = mysqli_fetch_assoc($result)) {
                    ?>
  ...
  <script src="js/honours/Honours_FPB.js"></script>
</body>
